from __future__ import annotations

from .utils import call_dequeue, call_enqueue, call_size, iso_ts, run_queue


def test_enqueue_size_dequeue_flow() -> None:
    run_queue([
        call_enqueue("companies_house", 1, iso_ts(delta_minutes=0)).expect(1),
        call_size().expect(1),
        call_dequeue().expect("companies_house", 1),
    ])


def test_rule_of_3_ordering() -> None:
    run_queue([
        call_enqueue("companies_house", 1, iso_ts(delta_minutes=10)).expect(1),
        call_enqueue("id_verification", 1, iso_ts(delta_minutes=10)).expect(2),

        call_enqueue("companies_house", 2, iso_ts(delta_minutes=10)).expect(3),
        call_enqueue("companies_house", 3, iso_ts(delta_minutes=10)).expect(4),

        call_enqueue("bank_statements", 1, iso_ts(delta_minutes=10)).expect(5),
        
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("id_verification", 1),
        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("companies_house", 2),
    ])


def test_temporal_ordering():
    # Ordering for 1 user
    run_queue([
        call_enqueue("companies_house", 1, iso_ts(delta_minutes=30)).expect(1),
        call_enqueue("id_verification", 1, iso_ts(delta_minutes=10)).expect(2),
        call_enqueue("bank_statements", 1, iso_ts(delta_minutes=20)).expect(3),

        call_dequeue().expect("id_verification", 1),
        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("companies_house", 1),
    ])
    
    # Ordering for multiple users
    run_queue([
        call_enqueue("bank_statements", 3, iso_ts(delta_minutes=30)).expect(1),
        call_enqueue("id_verification", 1, iso_ts(delta_minutes=10)).expect(2),
        call_enqueue("id_verification", 2, iso_ts(delta_minutes=60)).expect(3),
        call_enqueue("companies_house", 1, iso_ts(delta_minutes=20)).expect(4),
        call_enqueue("bank_statements", 4, iso_ts(delta_minutes=0)).expect(5),

        call_dequeue().expect("bank_statements", 4),
        call_dequeue().expect("id_verification", 1),
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("bank_statements", 3),
        call_dequeue().expect("id_verification", 2),
    ])


def test_dependency_resolution():
    # Dependency resolution
    run_queue([
        call_enqueue("credit_check", 1, iso_ts(delta_minutes=30)).expect(2),
        call_enqueue("credit_check", 2, iso_ts(delta_minutes=0)).expect(4),
        call_enqueue("bank_statements", 1, iso_ts(delta_minutes=10)).expect(5),
        
        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("credit_check", 1),
        call_dequeue().expect("companies_house", 2),
        call_dequeue().expect("credit_check", 2),
    ])


def test_multi_rule():
    run_queue([
        call_enqueue("credit_check", 1, iso_ts(delta_minutes=30)).expect(2),
        call_enqueue("bank_statements", 1, iso_ts(delta_minutes=10)).expect(3),
        call_enqueue("credit_check", 2, iso_ts(delta_minutes=0)).expect(5),
        call_enqueue("companies_house", 1, iso_ts(delta_minutes=20)).expect(6),
        call_enqueue("credit_check", 2, iso_ts(delta_minutes=40)).expect(8),
        call_enqueue("bank_statements", 4, iso_ts(delta_minutes=60)).expect(9),

        call_dequeue().expect("companies_house", 2),
        call_dequeue().expect("credit_check", 2),
        call_dequeue().expect("companies_house", 2),
        call_dequeue().expect("credit_check", 2),

        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("credit_check", 1),

        call_dequeue().expect("bank_statements", 4),
    ])
